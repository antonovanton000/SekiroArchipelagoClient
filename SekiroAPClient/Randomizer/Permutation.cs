...
namespace RandomizerCommon
{
    public class Permutation
    {
        ...
        public bool ForcedMode { get; private set; } = false;
        public void Forced(
                Dictionary<SlotKey, List<SlotKey>> items,
                Dictionary<SlotKey, List<SlotKey>> remove = null,
                Dictionary<SlotKey, int> quantities = null)
        {

            ForcedMode = true;

            // 1. Clear silos from the old logic
            ...
            // 2. Main permutation: targetSlot -> sourceSlots
            foreach (var kvp in items)
            {
                ...
                // Determine where this target lives (which ScopeType -> RandomSilo)
                ...
                foreach (SlotKey source in sources)
                {
                    ...
                    if (source.Scope.ID == 1120150)
                        source.ToString();

                    // Source within this silo (for stats/logging)
                    silo.Sources.Add(source);

                    // Actual mapping: target -> source
                    silo.Mapping[target].Add(source);

                    // If a quantities dictionary was provided,
                    // try assigning ForcedQuantity for this source.
                    if (quantities != null && quantities.TryGetValue(source, out int q) && q > 0)
                    {
                        var srcLoc = data.Location(source);
                        if (srcLoc != null)
                        {
                            srcLoc.ForcedQuantity = q;
                        }
                    }
                }
            }

            // 3. Handle remove: which slots should be cleaned up
            if (remove != null)
            {
                ...
                foreach (var kvp in remove)
                {
                    ...
                    // Prevent these targets from participating as normal targets
