...
namespace RandomizerCommon
{
    ...
    public class Util
    {
        ...
        public static void SetSingle<K, V>(IDictionary<K, List<V>> dict, K key, V value)
        {
            ...
            {
                list.Clear(); // <-- ключевой момент
            }

            list.Add(value);
        }

        public static void SetSelf<K>(IDictionary<K, List<K>> dict, K key)
        {
            if (!dict.TryGetValue(key, out var list))
            {
                list = new List<K>(1);
        ...
        public static void SetSingle<K, V>(IDictionary<K, HashSet<V>> dict, K key, V value)
        {
