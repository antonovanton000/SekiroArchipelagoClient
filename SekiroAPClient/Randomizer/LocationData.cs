...
namespace RandomizerCommon
{
    ...
    public class LocationData
    {
        ...
        {
            AddLocationlessItem(item, -1);
        }

        public void AddLocationlessItem(ItemKey item, int uniqueId)
        {
            ItemScope scope = new ItemScope(ItemScope.ScopeType.SPECIAL, uniqueId);
        ...
        public class ItemLocation
        {
            ...
            public int? ForcedQuantity { get; set; }
            public int Quantity
            {
                get
                {                    
                    if (ForcedQuantity.HasValue && ForcedQuantity.Value > 0)
                        return ForcedQuantity.Value;
                 
                    int min = Keys.Select(k => k.Quantity).DefaultIfEmpty().Min();
