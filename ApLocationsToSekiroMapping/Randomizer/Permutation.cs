...
namespace RandomizerCommon
{
    public class Permutation
    {
        ...
        public void Forced(
            Dictionary<SlotKey, List<SlotKey>> items,
            Dictionary<SlotKey, List<SlotKey>> remove = null)
        {
            // 1. Чистим всё, что могла нагенерить обычная логика (на всякий случай)
            foreach (var silo in Silos.Values)
            {
                silo.Sources.Clear();
                silo.Targets.Clear();
                silo.ExcludeTargets.Clear();
                silo.Mapping.Clear();
            }

            // 2. Заполняем маппинг "targetSlot -> sourceSlots" под нашу Archipelago-перестановку
            foreach (var kvp in items)
            {
                SlotKey target = kvp.Key;
                List<SlotKey> sources = kvp.Value;

                if (sources == null || sources.Count == 0)
                    continue;

                // Определяем, в какой силос положить этот таргет
                // (FINITE / INFINITE / MIXED / SHOP_INFINITE и т.п.)
                ItemLocation targetLoc = data.Location(target);
                if (targetLoc == null)
                    continue;

                ScopeType scopeType = targetLoc.Scope.Type;
                if (!canPermuteTo.TryGetValue(scopeType, out RandomSilo siloType))
                {
                    // На всякий случай: если тип неизвестен, считаем его FINITE
                    siloType = RandomSilo.FINITE;
                }

                SiloPermutation silo = Silos[siloType];

                // Для красоты/отчётности — регистрируем scope как "цель" этого силоса
                if (!silo.Targets.Contains(targetLoc.LocScope))
                {
                    silo.Targets.Add(targetLoc.LocScope);
                }

                // И сам таргет
                if (!silo.Mapping.ContainsKey(target))
                {
                    silo.Mapping[target] = new List<SlotKey>();
                }

                foreach (SlotKey source in sources)
                {
                    if (source == null)
                        continue;

                    // Источники в этом силосе (это больше для отладки/аналитики,
                    // Writer использует только Mapping)
                    silo.Sources.Add(source);

                    silo.Mapping[target].Add(source);
                }
            }

            // 3. Отдельно обрабатываем "remove" — то, что нужно удалить из игры
            if (remove != null)
            {
                SiloPermutation removeSilo = Silos[RandomSilo.REMOVE];
                ...
                foreach (var kvp in remove)
                {
                    ...
                    if (!removeSilo.Mapping.ContainsKey(target))
                    {
                        removeSilo.Mapping[target] = new List<SlotKey>();
                    }

                    foreach (SlotKey src in sources)
                    {
                        if (src == null)
                            continue;

                        removeSilo.Sources.Add(src);
                        removeSilo.Mapping[target].Add(src);
                    }

                    // Чтобы эти таргеты не участвовали в других силосах как "нормальные"
                    removeSilo.ExcludeTargets.Add(target);
                }
            }

            // 4. Никаких assign/KeyItemsPermutation мы тут НЕ трогаем.
            // В AP-режиме мы не вызываем Logic(), так что assign = null — и это нормально,
            // пока мы не используем подсказки/хинты, завязанные на логике ключевых предметов.
        }
